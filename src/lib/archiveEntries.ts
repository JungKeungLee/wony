...
const ARCHIVE_ENTRY_COLUMNS = "id,archive_id,date,title,description,tags,created_at,updated_at";

/** 등록된 모든 ARCHIVE 기록을 가져온다. 월별 그룹핑/정렬은 호출하는 쪽에서 한다. */
...
/**
 * 기록 한 건만 삭제한다. archive_images/archive_comments/Storage 파일은 절대 건드리지
 * 않는다 - 실수로 함께 지워지면 복구가 어렵기 때문에, 이미지/코멘트 삭제는 각각의
 * 기존 기능(대표 이미지 삭제, MEMORY NOTE 삭제)으로 따로 처리해야 한다.
 * RLS가 조건에 안 맞는 행을 조용히 0건 처리할 수 있으므로, 실제로 삭제된 행이
